...
ALTER TABLE Cliente ADD COLUMN compra VARCHAR(7) NOT NULL;

ALTER TABLE Cliente MODIFY num_orden VARCHAR(7);

ALTER TABLE Cliente ADD CONSTRAINT `compra` FOREIGN KEY (num_orden) REFERENCES PanelSolar(ID_panel);

ALTER TABLE Tecnico ADD COLUMN orden_servicio VARCHAR(7);

ALTER TABLE Tecnico ADD CONSTRAINT `instala` FOREIGN KEY (orden_servicio) REFERENCES PanelSolar(ID_panel);

ALTER TABLE Termico RENAME PanelTermico;
...
ALTER TABLE PanelTermico CHANGE auxilio sistema_auxiliar VARCHAR(5);

ALTER TABLE PanelTermico ADD CONSTRAINT `auxilia` FOREIGN KEY (sistema_auxiliar) REFERENCES Caldera(ID_cald);

ALTER TABLE PanelFotovoltaico ADD COLUMN bateria VARCHAR(4);

ALTER TABLE PanelFotovoltaico ADD CONSTRAINT `almacena` FOREIGN KEY (almacen_energ) REFERENCES Bateria(ID_bateria);
